...
struct ChannelTransportInner {
    send_channels: HashMap<SocketAddr, Sender<(SocketAddr, Message)>>,
...
/// Round-trips a message through serialization, to get closer to the real world.
fn serialize_deserialize_message(message: Message) -> Message {
    let buf = message.serialize_to_vec();
    assert_eq!(buf.len(), message.serialized_len());
    let mut read_cursor: &[u8] = &buf[..];
    let message_ser_deser = Message::deserialize(&mut read_cursor).unwrap();
...
impl ChannelTransport {
    ...
    async fn send(
        ...
        let message = serialize_deserialize_message(message);
...
struct InProcessSocket {
    ...
    message_rx: Receiver<(SocketAddr, Message)>,
}
